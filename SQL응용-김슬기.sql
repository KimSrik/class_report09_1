...
create table 영화(
    ...
    primary key(영화코드),
    foreign key(장르코드) references 장르(장르코드) on update cascade on delete cascade
 );
...
-- desc 감독;
 ...
 create table 디렉팅(
    ...
    foreign key(영화코드) references 영화(영화코드) on update cascade on delete cascade
 );
...
-- 6. 장르명이 'Fantasy'의 자료의 장르코드와 장르명을 각각 '000111', '판타지'로 변경.
-- * update의 위는 갱신하기 전을 확인하기 위한 select문, 아래는 갱신한 후 확인하기 위한 select문.
...
-- 												테이블 삭제
-- ------------------------------------------------------------------------------------------
-- 7. 장르가 '포르노'에 해당하는 영화의 모든 정보를 삭제.
-- * delete의 위는 갱신하기 전을 확인하기 위한 select문, 아래는 갱신한 후 확인하기 위한 select문.
